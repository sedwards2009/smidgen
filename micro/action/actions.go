...
func (h *BufPane) Search(str string, useRegex bool, caseSensitive bool, searchDown bool) (bool, error) {
	match, found, err := h.Buf.FindNext(str, h.Buf.Start(), h.Buf.End(), h.Cursor.Loc, searchDown, useRegex, caseSensitive)
	if err != nil {
		return false, err
	}
	if found {
...
func (h *BufPane) ReplaceAll(str string, useRegex bool, caseSensitive bool, replaceText string) (int, error) {
	if str == "" {
		return 0, nil
	}

	regex, err := buffer.CreateRegex(str, useRegex, caseSensitive)
	if err != nil {
		return 0, err
	}

	count, _ := h.Buf.ReplaceRegex(h.Buf.Start(), h.Buf.End(), regex, []byte(replaceText), false)
	return count, nil
}
...
func (h *BufPane) CutLine() bool {
	...
	if h.freshClip {
		if clip, err := clipboard.ClipboardReadAll(); err != nil {
			return false
		} else {
			clipboard.ClipboardWriteAll(clip + string(h.Cursor.GetSelection()))
		}
	} else {
		h.Cursor.CopySelection()
	}
...
func (h *BufPane) SpawnMultiCursor() bool {
	...
	match, found, err := h.Buf.FindNext(search, h.Buf.Start(), h.Buf.End(), searchStart, true, true, true)
	if err != nil {
		// InfoBar.Error(err)
	}
	if found {
...
func (h *BufPane) skipMultiCursor(forward bool) bool {
	...
	match, found, err := h.Buf.FindNext(search, h.Buf.Start(), h.Buf.End(), searchStart, forward, false, true)
	if err != nil {
		// InfoBar.Error(err)
	}
	if found {
